...
pixelPainter = (img) => (frame) => (ctx) => {
    ...
    for (let y = 0; y <= 1; y += hStep) {
        for (let x = 0; x <= 1; x += wStep) {
            ...
            const imageX = frameX * img.naturalWidth / frameWidth;
            const imageY = frameY * img.naturalHeight / frameHeight;
...
applyToCanvas(squareLimit(jeffTianPixel, 0)(f), '#square-limit-to-pixel', false);
```
